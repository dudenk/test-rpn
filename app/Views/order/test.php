<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>Welcome to CodeIgniter 4!</title>
	<meta name="description" content="The small framework with powerful features">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
	<link rel="shortcut icon" type="image/png" href="/favicon.ico"/>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" integrity="sha384-JcKb8q3iqJ61gNV9KGb8thSsNjpSL0n8PARn9HuZOnIxN0hoP+VmmDGMN5t9UJ0Z" crossorigin="anonymous">
    <style>
        header {
			background-color: rgba(247, 248, 249, 1);
			padding: .4rem 0 0;
		}
		.menu {
			padding: .4rem 2rem;
		}
		header ul {
			border-bottom: 1px solid rgba(242, 242, 242, 1);
			list-style-type: none;
			margin: 0;
			overflow: hidden;
			padding: 0;
			text-align: right;
		}
		header li {
			display: inline-block;
		}
		header li a {
			border-radius: 5px;
			color: rgba(0, 0, 0, .5);
			display: block;
			height: 44px;
			text-decoration: none;
		}
        section {
            margin:20px auto;
        }
        footer {
			text-align: center;
		}
		footer .copyrights {
			background-color: rgba(62, 62, 62, 1);
			color: rgba(200, 200, 200, 1);
			padding: .25rem 1.75rem;
		}
    </style>
   </head>
<body>

<!-- HEADER: MENU + HEROE SECTION -->
<header>

	<div class="menu">
		<ul>
			<li class="logo"><a href="https://codeigniter.com" target="_blank">
                <img height="44" title="CodeIgniter Logo" alt="Visit CodeIgniter.com official website!" src="data:image/png;base64,"></a>
			</li>
		</ul>
	</div>

</header>

<!-- CONTENT -->

<section  id="app" class="one-page">

<div class="container">
  <div class="card-deck mb-3 text-center">
    
    <div class="card mb-6 shadow-sm">
      <div class="card-header">
        <h4 class="my-0 font-weight-normal">Toko Online RPN</h4>
      </div>
      <div class="card-body">
        <div class="form-group">
            <label for="exampleFormControlSelect1">Pilih Product</label>
            <select class="form-control" v-model="dataOrder.productSelected" @change="calculate()">
                <option>-- Select --</option>
                <option v-for="(p, i) of products" v-bind:value="p">{{ p.name }}</option>
            </select>
        </div>
        <div class="form-group">
            <label for="Jumlah">Jumlah</label>
            <input type="number" class="form-control" placeholder="Jumlah" v-model="dataOrder.jumlah" @change="calculate()">
        </div>
        <div class="form-group">
            <label for="Jumlah">Discount</label>
            <div class="input-group mb-3">
                <input type="text" class="form-control" placeholder="0" v-model="dataOrder.discount" @change="calculate()">
                <div class="input-group-append">
                    <span class="input-group-text" id="basic-addon2">%</span>
                </div>
            </div>
        </div>
        <div class="form-group form-check">
            <input type="checkbox" class="form-check-input" v-model="dataOrder.ppn" v-bind:value="true" @change="calculate()">
            <label class="form-check-label" for="exampleCheck1" >PPN (10 %)</label>
        </div>
      </div>
      
      <div class="card-footer">
       
      </div>
    </div>

    <div class="card mb-6 shadow-sm">
      <div class="card-header">
        <h4 class="my-0 font-weight-normal">Konfirmasi Pembelian</h4>
      </div>
      <div class="card-body">
        <table class="table" width="100%">
            <tr>
                <td>Product</td><td>{{ dataOrder.productSelected.name }} ({{ formatPrice(dataOrder.productSelected.price) }}) </td>
            </tr>
            <tr>
                <td>Biaya Produk</td><td> {{ formatPrice(dataOrder.totalNilaiProduct) }} </td>
            </tr>
            <tr>
                <td>Total Discount</td><td> {{ formatPrice(dataOrder.potongan) }} </td>
            </tr>
            <tr>
                <td>Pajak</td><td> {{ formatPrice(dataOrder.pajak) }} </td>
            </tr>
            <tr>
                <td>Total</td><td> {{ formatPrice(dataOrder.total) }} </td>
            </tr>
        </table>
      </div>
      <div class="card-footer">
        <p class="info">{{ message }}</p>
        <div class="row">
            <div class="col-md-6"> <button type="button" class="btn btn-lg btn-block btn-primary" @click="beliPdf()">Pdf</button> </div>
        </div>
      </div>
    </div>
  </div>
    {{ message }}
</div>

</section>


<!-- FOOTER: DEBUG INFO + COPYRIGHTS -->

<footer>

	<div class="copyrights">

		<p>Page rendered in {elapsed_time} seconds. CI Version: <?= CodeIgniter\CodeIgniter::CI_VERSION ?> Environment: <?= ENVIRONMENT ?>. &copy; <?= date('Y') ?> CodeIgniter Foundation.</p>

	</div>

</footer>
<script src="https://cdn.jsdelivr.net/npm/axios/dist/axios.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/vue/dist/vue.js"></script>
<script>

const rpnApi = axios.create({
  baseURL: '<?= base_url()?>/',
  timeout: 1000,
  headers: {'X-Custom-Header': 'xxx'}
});

var app = new Vue({
  el: '#app',
  data: {
    message: '',
    products: <?php echo json_encode($products) ?>,
    dataOrder: {
        productSelected: {},
        ppn: false,
        discount: 0,
        potongan: 0,
        pajak: 0,
        jumlah: 0,
        totalNilaiProduct: 0,
        total: 0
    }
  },
  watch: {
  },
  mounted() {
  },
  methods: {
    formatPrice(value) {
        if ( value ) {
            let val = (value/1).toFixed(0).replace('.', ',')
            return 'Rp. ' + val.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")
        } else {
            return 'Rp. 0'
        }
    },
    calculate: function () {
        this.dataOrder.totalNilaiProduct = this.dataOrder.productSelected.price * this.dataOrder.jumlah;
        if ( this.dataOrder.discount > 0 ) {
            this.dataOrder.potongan = this.dataOrder.totalNilaiProduct * ( this.dataOrder.discount / 100 );
        } else {
            this.dataOrder.potongan = 0;
        }
        if ( this.dataOrder.ppn ) {
            this.dataOrder.pajak = ( this.dataOrder.totalNilaiProduct - this.dataOrder.potongan ) *   <?= $ppn ?> ; 
        } else {
            this.dataOrder.pajak = 0;
        }
        this.dataOrder.total = this.dataOrder.totalNilaiProduct - this.dataOrder.potongan + this.dataOrder.pajak;
        console.log(this.dataOrder)
    },
    beliPdf: function () {
        if ( this.dataOrder.total > 0 ) {
            rpnApi.post('order/save', this.dataOrder )
            .then(response => (
                // this.message = response.data
                window.open("<?= base_url();?>/order/print/"+response.data.data, "_blank")
            ))
        } else {
            this.message = "Mohon Periksa Order, Data Tidak Valid"
        }
        
    },
    beliSimpan: function () {
        rpnApi.post('order/save')
        .then(response => (
            this.message = response.data 
        ))
    }
  }
});
</script>

</body>
</html>
